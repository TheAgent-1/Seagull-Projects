...
bool RFID_LOOP() {
  ...
  */
  RFID_READCARD(block, readbackblock);

  CardCount++;
  return true;
}

int RFID_READCARD(int blockNumber, byte arrayAddress[]) {
...
void LCD_LOOP(int number, int col, int row) {
  lcd.setCursor(col, row);
  lcd.print(number);
}
